...
export class AllProduitComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    /*on store la liste des produits si pas de catégorie sélectionnée alors on affiche tout les produits*/
    if (this.categorieSelectionnee === undefined){ this.AllProducts();}
    /*sinon on affiche uniquement les produits désirés*/
    else{ this.filterCategorie(this.categorieSelectionnee);}/* */
  ...
  filterCategorie(id : string): void{
      try {
        ...
        this.liste_cate.forEach((categorie) => {
          categorie.active = categorie.id === id;
          })
